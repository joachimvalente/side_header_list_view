...
class _SideHeaderListViewState extends State<SideHeaderListView> {
  ...
  static double _controllerOffset = 0.0;
  ...
  void _reposition() {
    print('in reposition');
    setState(() => _controllerOffset = _controller.offset);
  }

  @override
  Widget build(BuildContext context) {
    print('in build');
    final currentPosition = (_controllerOffset / widget.itemExtent).floor();
    return Stack(
      children: <Widget>[
        Positioned(
          child: Opacity(
            opacity:
                _shouldShowHeader(currentPosition, currentPosition) ? 0.0 : 1.0,
          ...
          itemBuilder: (BuildContext context, int index) => Row(
            ...
            children: <Widget>[
              FittedBox(
                child: Opacity(
                  opacity:
                      _shouldShowHeader(index, currentPosition) ? 1.0 : 0.0,
  ...
  bool _shouldShowHeader(int position, int currentPosition) =>
      (position < 0) ||
